...
array=(${TARBALL_NAME//-/ })
ORIGINAL_DB_NAME=${array[0]}
echo $ORIGINAL_DB_NAME
...
docker exec -it $CONTAINER_NAME  bin/neo4j-admin database restore --from-path=$BACKUP_FOLDER/$TARBALL_NAME --overwrite-destination $DATABASE_NAME
